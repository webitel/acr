...
import (
	...
	"errors"
	"fmt"
	"github.com/webitel/acr/src/provider/fs/fs_reader"
	"io"
	"log"
	"net"
	"net/url"
...
const eventsBuffer = 32      // For the events channel (memory eater!)
const timeoutPeriod = 20 * time.Second
...
func Listen(srv net.Listener, fn HandleFunc) error {
	for {
		c, err := srv.Accept()
		if err != nil {
			return err
...
func (h *Connection) SendEvent(m map[string]string, name string) error {
	var err error
